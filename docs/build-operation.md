...
!!! Note
    CubeMX itself does not have the required interfaces to the csolution project format. The utility `cbridge` converts the [build information files](YML-CBuild-Format.md) into command-line options for CubeMX. `cbridge` also generates the `<project-name>.cgen.yml` based on the information generated by CubeMX.
