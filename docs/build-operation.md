...
- [Native CMake Project Integration](#native-cmake-project-integration) describes direct orchestration of CMake projects.
- [CMake Script Integration with `executes:`](#cmake-script-integration-with-executes) describes CMake-based custom build steps.
...
```bash
cbuild <name>.csolution.yml --active <target-type>[@<target-set>] [--packs] [--update-rte] [--frozen-packs]
```
...
- It calls [`csolution`](build-tools.md#csolution-invocation) to process the the `<name>.csolution.yml` project.
    ...
    - With option `--active` a [target with optional set](build-overview.md#working-with-target-set) selects the projects and build-types that form the application.
...
```bash
cbuild setup <name>.csolution.yml --active <target-type>[@<target-set>] [--packs] [--update-rte] [--frozen-packs]
```

The option `--active` selects the [target-set](build-overview.md#working-with-target-set) that specifies the application configuration. For an application described by a `<name>.csolution.yml` file, these steps are executed:
...
- Create the files `compile_commands.json` and `compile_macros.h` in the [output directory](build-overview.md#output-directory-structure) for every context that belongs to the active target-set.

!!! Note
    The file `compile_commands.json` is generated by `CMake` with the target `<context>-database` and contains all source files of the context along with compiler options. The generated `compile_macros.h` records the selected toolchain's built-in macros and is included by the compilation database commands. This makes the macros available to code-analysis tools that consume only `compile_commands.json`, without requiring IDE-specific configuration. These files are used by various code analyzer tools and the [IntelliSense](https://code.visualstudio.com/docs/editor/intellisense) system of an IDE. For practical desktop and CI workflows, refer to [Static Code Analysis](build-tools.md#static-code-analysis).
...
- CMake build command automatically executed by `cbuild setup` to generate `compile_commands.json` for each [context](build-overview.md#context):
...
## Native CMake Project Integration

CMSIS-Toolbox 2.15 can orchestrate a native CMake project directly from a [`cmake:` project entry](YML-Input-Format.md#cmake-build). Such an entry creates a synthetic project context and generated `*.cbuild.yml` metadata, but bypasses processing that is specific to `*.cproject.yml` files, such as CMSIS component, translation-control, and linker-script generation.

For each selected native CMake context, `cbuild` uses the specified source directory and generator, appends the entries under `configure:` to the CMake configure operation, and builds the optional `target:`. If `target:` is omitted, the native CMake project's default target is built. The normal clean and rebuild operations apply to these contexts. To generate a compilation database, add `-DCMAKE_EXPORT_COMPILE_COMMANDS=ON` under `configure:`.

Files declared under `images:` are treated as outputs of the native build. They are copied into the generated context information and, when selected by a `target-set:`, into `*.cbuild-run.yml` for programming and debugging. The optional `device:` identifies the processor that executes those images in a multi-processor target.

This direct project integration differs from the following mechanism, which invokes CMake only as a custom build step.

## CMake Script Integration with `executes:`
...
The west build system uses two environment variables to configure the toolchain. These variables are set when the `*.csolution.yml` contains the [`west:`](YML-Input-Format.md#west) node. The cbuild orchestration sets them when West is called during setup and build operations; the VS Code CMSIS Solution extension also provides them in its build environment.
...
The master definitions are maintained in the [Open-CMSIS-Pack Debug Adapter Registry](https://github.com/Open-CMSIS-Pack/debug-adapter-registry) and deployed with CMSIS-Toolbox. Support for a new or updated third-party debug adapter should be contributed to that registry rather than added to an application project or a local CMSIS-Toolbox installation.
