...
## GNU Debugger Issues with AC6 or CLANG

GNU debugger components may have issues with ELF files generated by AC6 or CLANG. The following sections describe known problems and workarounds.

### Stack Unwinding Fails or Hangs

AC6 or CLANG may not emit the required ELF/DWARF unwind information for functions with the `noreturn` attribute. As a result, GDB may show an incorrect call stack or the stack unwinder may take a long time or hang.

The compiler option `-funwind-tables` preserves the link register on the stack, which slightly increases stack usage but enables reliable stack unwinding. Add the option only to the debug `build-type` in the `*.csolution.yml` file.

#### AC6

Apply the workaround to C files compiled with AC6:
...
Apply the workaround to C files when the selected CLANG version exhibits the issue:
...
!!! Caution
    Do not add this option to [`cdefault.yml`](build-overview.md#cdefaultyml), as it changes the generated code for all build types, including release builds.
...
To solve this problem, change to DWARF-5 format. The format is typically specified in the [`cdefault.yml`](build-overview.md#cdefaultyml) file.  You may create a local copy of this file in the same directory as the `*.csolution.yml` file.
