...
[`*.cbuild-set.yml`](#cbuild-setyml)            | Legacy context selection for the build process, enabled with option [`--context-set`](build-tools.md#legacy-context-set).
...
!!! Note
    - CMSIS-Toolbox 2.11 or later creates the `*.cbuild.yml` and `*.cbuild-run.yml` files in the `out` directory along with the related output files.
    - The `*.cbuild-idx.yml` file is located in the same directory as the `*.csolution.yml` file and provides the paths to the `*.cbuild.yml` and `*.cbuild-run.yml` files.
...
The `resolved-pack:` node uses the canonical vendor and pack-name spelling from the PDSC file, while `selected-by-pack:` preserves the spelling used in the project input. If *csolution project files* are modified, the `selected-by-pack` information ensures that consistent pack versions are used. If a required pack is no longer used or present in the *csolution project*, the relevant entry `resolved-pack:` in  the `*.cbuild-pack.yml` file is removed.
...
&nbsp;&nbsp;&nbsp; `target:`                              | Selected target in the format `<target-type>[@<target-set>]`.
...
&nbsp;&nbsp;&nbsp; [`cbuild-run:`](#run-and-debug-management) | Path to the `*.cbuild-run.yml` file generated for the selected target-set.
...
build-idx:
  ...
  cbuilds:
    ...
    - cbuild: out/hello_world/B-U585I-IOT02A/Release/hello_world.Release+B-U585I-IOT02A.cbuild.yml
      ...
      project: hello_world
      configuration: .Release+B-U585I-IOT02A
    - cbuild: out/native/B-U585I-IOT02A/Release/native.Release+B-U585I-IOT02A.cbuild.yml
      cmake: true
      project: native
      configuration: .Release+B-U585I-IOT02A
  cbuild-run: out/USB+B-U585I-IOT02A.cbuild-run.yml
...
The `<solution-name>.cbuild-set.yml` file stores a legacy [context selection](build-tools.md#legacy-context-set) for the *csolution project*. New projects should define a [`target-set:`](YML-Input-Format.md#target-set) and select it with `--active`. The legacy file structure is outlined below.

`cbuild-set:`                                                      | Content
:------------------------------------------------------------------|:------------------------------------
&nbsp;&nbsp;&nbsp; `generated-by:`                                 | Reference to tool along with version information that generated this file.
&nbsp;&nbsp;&nbsp; `contexts:`                                     | List of [context](YML-Input-Format.md#context-name-conventions) names for the legacy [`--context-set`](build-tools.md#legacy-context-set) option.
...
cbuild-set:
  ...
  contexts:
    - context: CM33_s.Release+FVP
    - context: CM33_ns.Debug+FVP
```

### `*.cbuild-mlops.yml`

The `<solution-name>.cbuild-mlops.yml` file contains parameters for [MLOps integration](build-overview.md#mlops-integration). This file is generated when the [`mlops:`](YML-Input-Format.md#mlops) node is present in the `*.csolution.yml` file. The intent is to provide ML model and NPU parameters (including Vela configuration for Ethos-U) and information for build and test environments.

Refer to [MLOps Integration](build-overview.md#mlops-integration) for more information.
...
For each native CMake project context a node `cmake:` is created in `*.cbuild.yml`.

`cmake:`                                                  |              | Content
:---------------------------------------------------------|:-------------|:------------------------------------
&nbsp;&nbsp;&nbsp; `source:`                              | **Required** | Path to the CMake source directory.
&nbsp;&nbsp;&nbsp; `project-id:`                          | **Required** | Project identifier.
&nbsp;&nbsp;&nbsp; `generator:`                           |   Optional   | CMake generator used for the build.
&nbsp;&nbsp;&nbsp; `configure:`                           |   Optional   | List of options for the CMake configure command.
&nbsp;&nbsp;&nbsp; `target:`                              |   Optional   | CMake build target.
&nbsp;&nbsp;&nbsp; `images:`                              |   Optional   | List of output images produced by the CMake project.
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; `- image:`           | **Required** | Path to the CMake build output image.
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; `type:`  | **Required** | Output image type: `elf`, `hex`, `bin`, or `lib`.
...
This node contains information about the images that should be loaded. The images that are generated by the *csolution project* are typically configured [using a target-set](build-overview.md#working-with-target-set). The `output:` node includes also image files that are generated with a [West Build](YML-Input-Format.md#west-build) or [CMake Build](YML-Input-Format.md#cmake-build) specification.
...
`scvd`  | [Software Component Viewer Description (`*.scvd`) file](https://arm-software.github.io/CMSIS-View/latest/SCVD_Format.html) for [CMSIS-View](https://arm-software.github.io/CMSIS-View/latest/index.html).
...
&nbsp;&nbsp;&nbsp; `rtt:`                                 |  Optional   | Segger RTT configuration.
&nbsp;&nbsp;&nbsp; `systemview:`                          |  Optional   | Segger SystemView configuration.
