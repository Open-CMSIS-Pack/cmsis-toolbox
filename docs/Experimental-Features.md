...
- When [`resources:`](#resources) node is specified in one of the `*.cproject.yml` or `*.clayer.yml` files of a *csolution project*:
  ...
  - The file `.\cmsis\<solution-name>+<target-name>.regions.h` replaces the `regions_<device_or_board>.h` that is  located in the directory `./RTE/Device/<device>`. The `regions_<device_or_board>.h` is not longer generated.
...
The `resources:` node specifies the resources required by a project. It be used at the level of `project:`, `setup:`, or `layer:`. The `resources:` node is additive; when multiple `resources:` nodes specify the same region the size is added.

```yml
  resources:
    regions:
      - region: __ROM0    # pre-defined region name: __ROM0..3
        size: 0x10000     # specifies region size
#       name:             - name of the physical memory the region should belong too
#       address:          - absolution address of region; not in scope for 2.7
#       startup:          - control usage of startup region; not in scope for 2.7
#       align:            - alignment restrictions of the regions; not in scope for 2.7

      - region: __RAM0    # pre-defined region name: __RAM0..3
        size: 0x8000      # specifies region size
#       name:             - name of the physical memory the region should belong too
#       address:          - absolution address of region; not in scope for 2.7
#       align:            - alignment restrictions of the regions; not in scope for 2.7

    heap:  0x2000         # heap size; question do we need to control the region that it belongs too?
    stack: 0x4000         # stack size; question do we need to control the region that it belongs too?
```
...
// Generated using information from packs and USBD.csolution.yml
...
#define __ROM0_BASE  0x08100000      /* Memory Name: Flash */
...
- Is it possible to provide a "generic way" to assign source files to sections?  
- Should regions directly map to physical memory names?
